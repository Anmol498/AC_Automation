...
const App: React.FC = () => {
  const [auth, setAuth] = useState<AuthState>(() => {
    const saved = localStorage.getItem('satguru_engineers_auth');
  ...
  const login = (user: User, token: string) => {
    ...
    setAuth(newState);
    localStorage.setItem('satguru_engineers_auth', JSON.stringify(newState));
  };

  const logout = () => {
    setAuth({ user: null, token: null, isAuthenticated: false });
    localStorage.removeItem('satguru_engineers_auth');
  };
