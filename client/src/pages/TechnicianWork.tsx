...
export default function TechnicianWork() {
    ...
    const [editForm, setEditForm] = useState({ date: '', work_description: '', qty: '', remarks: '', address: '' });

    // New row form
    const [newRow, setNewRow] = useState<Partial<WorkLog>>({ date: new Date().toISOString().split('T')[0], work_description: '', qty: '1', remarks: '', address: '' });
    ...
    const exportToCSV = () => {
        ...
        link.setAttribute('download', `${isSuperAdmin ? 'All_Technician' : 'My'}_Work_Logs_${new Date().toISOString().split('T')[0]}.csv`);
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100 bg-white">
                            ...
                            {/* Loading */}
                            {isLoading && (
                                <tr><td colSpan={isSuperAdmin ? 7 : 7} className="p-8 text-center text-slate-400"><i className="fa-solid fa-spinner fa-spin mr-2"></i>Loading...</td></tr>
                            )}
